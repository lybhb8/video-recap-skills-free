...
def transcribe_audio(video_path, work_dir):
    """提取音频并用配置的 ASR 引擎分段转录，通过分段合成时间戳。"""
    asr_file = work_dir / "asr_result.json"

    if CONFIG.get("asr_engine") != "funasr" and not CONFIG.get("mimo_asr_api_key"):
...
def _run_asr_funasr(wav_path):
    """用本地 llama-funasr-sensevoice (GGUF) 转录单个 wav，返回纯文本。

    CLI stdout 即转写文本（UTF-8），stderr 是 [sensevoice] 日志；rc=0 即成功。
    失败抛错，避免把瞬时失败缓存成空转写。
    """
    binary = CONFIG.get("funasr_bin")
    model = CONFIG.get("funasr_model")
    if not binary or not model:
        raise RuntimeError(
            "FunASR 未配置：请设置 FUNASR_BIN 与 FUNASR_MODEL（ASR_ENGINE=funasr 需要）。"
        )
    # FUNASR_BIN 可以是可执行文件路径，也可以只给目录（自动拼默认可执行名）。
    binary = Path(binary)
    if binary.is_dir():
        binary = binary / ("llama-funasr-sensevoice.exe" if os.name == "nt" else "llama-funasr-sensevoice")
    cmd = [str(binary), "-m", str(model), "-a", str(wav_path)]
    vad = CONFIG.get("funasr_vad")
    if vad:
        cmd += ["--vad", str(vad)]
    result = run_cmd(cmd)
    if result.returncode != 0:
        raise RuntimeError(f"FunASR 转录失败: {result.stderr}")
    return (result.stdout or "").strip()


def _run_asr(wav_path):
    """用配置的 ASR 引擎转录单个 wav 文件，返回纯文本。

    非 funasr 时走 MiMo ASR (mimo-v2.5-asr)：音频以 base64 data-URI 放进 OpenAI 风格
    的 chat/completions 消息里，转写文本回到 choices[0].message.content。API/响应结构
    失败会抛错，避免把瞬时失败缓存成空转写；只有无音频、超体积等确定不可发送的片段
    返回空串。
    """
    if CONFIG.get("asr_engine") == "funasr":
        return _run_asr_funasr(wav_path)
    try:
