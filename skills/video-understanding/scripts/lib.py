...
def load_user_environment():
    """Merge Windows User-scope registry environment variables into os.environ.

    PowerShell's [Environment]::SetEnvironmentVariable(..., 'User') writes to the
    registry only; already-running processes keep their old environment until
    restarted. Calling this at import time lets config set that way (MIMO_*,
    ASR_ENGINE, TTS_ENGINE, FUNASR_*, ...) take effect without opening a new
    terminal. Process-level variables already in os.environ win over the registry.
    Set VIDEO_RECAP_LOAD_USER_ENV=0 to disable (used by the test runner).
    """
    if os.name != "nt":
        return
    if os.environ.get("VIDEO_RECAP_LOAD_USER_ENV", "1").strip().lower() in ("0", "false", "no"):
        return
    try:
        import winreg
        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment") as key:
            ...
            while True:
                ...
                if isinstance(name, str) and isinstance(value, str):
                    os.environ.setdefault(name, os.path.expandvars(value))
    except OSError:
        pass
...
CONFIG = {
    ...
    "auth_scheme": os.environ.get("MIMO_AUTH_SCHEME", "api-key"),  # api-key (MiMo) | bearer (GLM)
    "asr_engine": os.environ.get("ASR_ENGINE", "mimo"),  # mimo (云端 MiMo) | funasr (本地 SenseVoice)
    "funasr_bin": os.environ.get("FUNASR_BIN", ""),  # llama-funasr-sensevoice 可执行文件
    "funasr_model": os.environ.get("FUNASR_MODEL", ""),  # sensevoice-small-q8.gguf
    "funasr_vad": os.environ.get("FUNASR_VAD", ""),  # fsmn-vad.gguf
    ...
    # 单请求最多携带的图片数。GLM-4V-Flash 等 bearer 端点实测上限 5 张（超限报 1210/参数错误），
    # 超长场景按此分批发送、跨批合并；MiMo (api-key) 默认保持 16 不改变原行为。
    "vlm_max_images_per_request": env_int(
        "VLM_MAX_IMAGES_PER_REQUEST",
        5 if os.environ.get("MIMO_AUTH_SCHEME", "api-key").lower() == "bearer" else 16,
        minimum=1,
    ),
...
def _api_headers(api_provider=None, api_url=None, api_key=None):
    """Build auth headers (api-key for MiMo; Bearer when MIMO_AUTH_SCHEME=bearer)."""
    del api_provider, api_url  # single provider per config; signature kept for call sites
    key = CONFIG.get("api_key", "") if api_key is None else api_key
    scheme = str(CONFIG.get("auth_scheme") or "api-key").lower()
    if scheme == "bearer":
        return {
...
def _prepare_api_payload(payload, api_provider=None, api_url=None):
    """Normalize payload fields for the configured chat/completions API."""
    del api_provider, api_url
    normalized = dict(payload)
    scheme = str(CONFIG.get("auth_scheme") or "api-key").lower()
    if scheme != "bearer":
        # MiMo API: max_tokens → max_completion_tokens, thinking disabled by default
