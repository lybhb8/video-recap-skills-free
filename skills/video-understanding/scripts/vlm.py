...
from lib import log, load_prompt, mimo_video_api_call, run_cmd, file_fingerprint, stable_hash
...
def analyze_scenes(scenes, frames, work_dir, *, resume=True):
    ...
    def _analyze_single_scene(i, scene):
        ...
        # GLM-4V-Flash 等 bearer 端点单请求图片数受限（vlm_max_images_per_request，默认 5）。
        # 超限场景拆成多批发送，跨批合并描述/帧标签/深层分析，保留原采样密度。
        per_request = max(1, int(CONFIG.get("vlm_max_images_per_request") or 5))
        batches = [scene_frames[k:k + per_request] for k in range(0, len(scene_frames), per_request)]

        log(f"VLM 分析场景 {i+1}/{len(scenes)} ({len(scene_frames)} 帧, {len(batches)} 批)...")
        ...
        for batch_no, batch in enumerate(batches, start=1):
            ...
            # 帧级事实标签：将帧时间注入prompt
            frame_ts_list = [f"{frame_times[f]:.1f}s" for f in batch]
            ...
            for attempt in range(3):
                ...
                if attempt < 2:
                    log(f"  场景 {i+1} 批 {batch_no} VLM 返回空，重试 ({attempt+2}/3)...")
            ...
            if depth_analysis:
                merged_depth.append(depth_analysis)
            for ts, actions in frame_facts.items():
                merged_facts.setdefault(ts, actions)
        ...
        result = {
            ...
            "start": scene["start"],
            "end": scene["end"],
            "description": "；".join(dict.fromkeys(merged_descriptions)) or "(VLM 无法识别此场景画面)",
            "depth_analysis": "；".join(merged_depth),
        }
        if merged_facts:
            result["frame_facts"] = merged_facts
    ...
    def _scene_cache_key(i, scene):
        ...
        return "|".join(str(x) for x in (
            ...
            CONFIG.get("vlm_max_images_per_request"),
