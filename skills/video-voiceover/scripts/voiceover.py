...
SUPPORTED_TTS_ENGINES = {"mimo-tts", "edge-tts"}
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if engine == "mimo-tts":
                ...
                _tts_edge_tts(text, output_wav, rate=rate, pitch=pitch, emotion=emotion)
            else:
                raise RuntimeError(
                    f"不支持的 TTS 引擎: {engine}。当前仅支持 mimo-tts、edge-tts。"
                )
...
def _detect_tts_engine():
    """解析 TTS 引擎：显式配置 edge-tts 时无需 MiMo key；否则 MiMo TTS 需要 key。"""
    if CONFIG.get("tts_engine") == "edge-tts":
        return "edge-tts"
...
def tts_settings_fingerprint(engine=None):
    ...
    if resolved == "edge-tts":
        settings.pop("mimo_tts_api_url", None)
        settings.pop("mimo_tts_model", None)
        settings.pop("mimo_tts_voice", None)
        settings.pop("mimo_tts_style", None)
        settings["edge_tts_voice"] = CONFIG.get("edge_tts_voice")
        return settings  # edge-tts 无 voiceclone 路径
    voice_ref = str(CONFIG.get("voice_ref") or "").strip()
    if voice_ref:
...
def _tts_edge_tts(text, output_wav, rate="+0%", pitch="+0Hz", emotion=None):
    """使用 Microsoft Edge TTS（免费，无需 API key）合成解说音频。

    edge-tts 走微软在线语音合成，rate/pitch 格式（"+5%"/"+3Hz"）与管线其余
    部分完全一致；emotion 无对应参数，忽略。输出为 MP3，转成管线统一的
    24kHz 单声道 PCM WAV 后写回 output_wav。
    """
    del emotion  # edge-tts 无情绪参数；解说情绪由文案与语气本身承载
    try:
        import edge_tts
    except ImportError as exc:
        raise RuntimeError(
            "edge-tts 未安装：请先 `pip install edge-tts`（TTS_ENGINE=edge-tts 需要）。"
        ) from exc
    voice = CONFIG.get("edge_tts_voice") or "zh-CN-XiaoxiaoNeural"
    wav_path = Path(output_wav)
    ...
    try:
        ...
        if not mp3_path.is_file() or mp3_path.stat().st_size <= 0:
            raise RuntimeError("edge-tts 未生成音频")
        result = run_cmd([
            "ffmpeg", "-y", "-i", str(mp3_path),
            "-ar", "24000", "-ac", "1", "-acodec", "pcm_s16le", str(wav_path),
        ])
        if result.returncode != 0 or not wav_path.is_file() or wav_path.stat().st_size <= 44:
            detail = (result.stderr or "").strip()
            raise RuntimeError(f"edge-tts 音频转码失败: {detail or mp3_path}")
    finally:
        try:
            if mp3_path.is_file():
                mp3_path.unlink()
